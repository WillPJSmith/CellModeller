...
# importing contact-dependent killing (cdk) module
import CDK_Module_V2 as cdk
...
"""
    ...
    def next_idx(self):
		...
		# new system recycling dead cells
		dead_cells = self.phys.isDeleted
		if numpy.count_nonzero(dead_cells) == 0:    
			idx = self._next_idx
			self._next_idx += 1
			return idx
		else:
			idx = numpy.nonzero(dead_cells)[0][0]
			self.phys.clear_cell(idx)
			return idx
    ...
    def init(self, phys, reg, sig, integ, CDKParams=None):
		self.phys = phys
		self.reg = reg
		self.sig = sig
		...
		if CDKParams:
			print 'Adding CDK object...'
			self.cdk = cdk.CDK_Module(CDKParams)
			
			# pass cdk copies of our OpenCL context and queue
			self.cdk.context = self.phys.context
			self.cdk.queue = self.phys.queue
		
			# initialise program and data
			self.cdk.init_cl()
			self.cdk.init_data()  
	...
	def kill(self, state):
		...
		# dead cells can't grow or divide
		state.growthRate = 0
		state.divideFlag = False

		# remove the dead cell
		self.phys.delete(state.idx)
		del self.cellStates[state.id]
    ...
    def step(self):
		...
		for (cid,state) in states.items():
			...
			# kill cells marked for death
			if state.deathFlag:
				self.kill(state)
			# divide cells marked for division
			if state.divideFlag:
		...
		# check cdk interactions, if using
		if self.cdk:
			# trigger cell firings
			self.reg.module.FiringProtocol(self.cellStates)

			# update cells states based on needle hits
			self.cdk.step(self.cellStates)
